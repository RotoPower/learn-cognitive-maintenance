...
- [~] D2.2 ingest Worker (`apps/ingest/`): **built and tested** (8 vitest tests in workerd: one row per tag per sim hour, idempotent on (tag, ts), hour-by-hour advance, gap backfill after a clock jump bounded by `MAX_BACKFILL_HOURS`, NULL for outages, loud failure on bad token, `/health`, admin-only `POST /ingest`). Shares the plant-api D1 database. **Remaining, yours**: `wrangler secret put READ_TOKEN/ADMIN_TOKEN --env staging` in `apps/ingest`, `npm run deploy:staging`, then set the plant clock to speed 3600
