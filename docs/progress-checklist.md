...
- [x] D2.1 port `plant/sim.py` to a Worker + Durable Object clock + D1 schema (`apps/plant-api/`): **deployed to staging** at `https://plant-api-staging.rotopower.workers.dev` (D1 `plant-staging`, secrets set). Verified with `plantctl`: all READ and ADMIN routes, readings byte-identical to the local sim, outage null, work order persisted, both artefacts uploaded, clock jump/speed. Gotcha: Cloudflare's Browser Integrity Check blocks the default `Python-urllib` user agent with a 403; all repo HTTP clients now send their own User-Agent. Detail: `src/sim.ts` reproduces the Python sim bit-for-bit (blake2b over the same key strings; 828-sample parity fixture from `scripts/export_parity_fixture.py`), `src/clock.ts` is the Clock DO (anchor + speed, computed on read), `migrations/0001_schema.sql` has all D1 tables, `src/index.ts` serves the same routes/tokens as `plant/api.py`. 27 vitest tests in workerd (Miniflare D1 + DO). `wrangler deploy --dry-run --env staging` bundles. **Remaining, yours**: `wrangler login`, `wrangler d1 create plant-staging` (+ paste id), migrate, `wrangler secret put READ_TOKEN/ADMIN_TOKEN --env staging`, `npm run deploy:staging` (see `apps/plant-api/README.md`)
- [x] D2.2 ingest Worker (`apps/ingest/`): **deployed to staging**, `plant-ingest-staging`. 9 vitest tests (one row per tag per sim hour, idempotent on (tag, ts), hour-by-hour advance, gap backfill bounded by `MAX_BACKFILL_HOURS`, first-run seed of `INITIAL_BACKFILL_HOURS`, NULL for outages, loud failure on bad token, `/health`, admin-only `POST /ingest`). Verified in D1: 24 rows per hour, no nulls, continuous hours at clock speed 3600. Two gotchas fixed: (a) a Worker cannot fetch another Worker's `workers.dev` URL on the same account (error 1042), so ingest calls the API through a Service Binding; (b) the running clock did not stop at the horizon and produced 2025 readings; the API now clamps sim time to the horizon end, and the stray rows were deleted. Pending redeploy of both Workers for the clamp and the first-run seed
